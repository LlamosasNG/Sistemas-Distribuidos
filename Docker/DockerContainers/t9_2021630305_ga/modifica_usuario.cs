            ...
            HttpRequest req)
        {
            try
            {
                ...
                try
                {
                    ...
                    cmd_1.CommandText = @"UPDATE usuarios SET password=@password, nombre=@nombre, apellido_paterno=@apellido_paterno, 
                    ...
                    // 2. Eliminar foto existente del usuario
                    ...
                    // 3. Insertar nueva foto si se proporciona
                    if (usuario.foto != null)
                    {
                        MySqlCommand cmd_3 = new MySqlCommand();
                        cmd_3.Connection = conexion;
                        cmd_3.Transaction = transaccion;
                        cmd_3.CommandText = @"INSERT INTO fotos_usuarios(id_foto, foto, id_usuario) 
                                             VALUES (0, @foto, (SELECT id_usuario FROM usuarios WHERE email = @email))";
                        
                        cmd_3.Parameters.AddWithValue("@foto", Convert.FromBase64String(usuario.foto));
